...
The shell commands such as `xsel`, `pbcopy`, and `clip` have several differences, accessing the clipboard in a procedural way occurs side effects, and [Clipboard](//hackage.haskell.org/package/Clipboard) in the Hackage can't deal with non-string data.  Offhand, I have tried wrapping an access the clipboard by Java with `scalaz.effect.IO` for these reasons.
