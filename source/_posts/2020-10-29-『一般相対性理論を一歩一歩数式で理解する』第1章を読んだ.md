...
ベクトル積が導入され、交換法則[^fn-1] {% katex %} a \times b = -(b \times a) {% endkatex %} と分配法則が紹介されています。

本には明記されていませんが、この記事では、ベクトル積は実数成分の3次元ベクトル {% katex %} \mathbb{R}^3 {% endkatex %} 上の演算であると見なします。実際、交換法則と分配法則を証明しようとすると、成分の集合において、加法が交換法則と結合法則を満たし、乗法が交換法則と加法に対する分配法則を満たすことが分かります。

[^fn-1]: 「交代性を持つ」と言った方が良い気がする。

ベクトル積に関するいくつかの定理が紹介されています。 {% katex %} a \times a = 0 {% endkatex %} を証明しておきます。:
...
節の終わりに、内積とベクトル積に関する公式が紹介されています。

## §2 微分の公式

積の微分法則 {% katex %} (f(x) g(x))' = f'(x) g(x) + f(x) g'(x) {% endkatex %} と連鎖律 {% katex %} (f(g(x)))' = f'(g(x)) g'(x) {% endkatex %} が紹介され、これらの法則がベクトル値関数に拡張されたり、連鎖律が多変数関数に拡張されたりしています。

<!-- TODO: 多変数関数の連鎖律の証明 -->

## §3 3次元の座標変換

直交変換が導入され、直交変換が内積を保存することが示されています。

## §4 スカラー場，ベクトル場のイメージ

スカラー場とベクトル場が簡単に紹介されています。関数の微分可能性について、 _何回でも微分可能な関数しか扱わない_ との註釈があります。

## §5 勾配

勾配が導入され、2次元スカラー場 {% katex %} f {% endkatex %} の勾配が {% katex %} f {% endkatex %} の傾きの最大値を表すことが説明されています。

p.39 (2) の左図において、 {% katex %} (a, b) {% endkatex %} の点は、直線 {% katex %} l {% endkatex %} 上で {% katex %} t = 0 {% endkatex %} の点です。

p.40 に <q>{% katex %} f(x, y) {% endkatex %} で， {% katex %} x = a + t \cos \theta {% endkatex %} ， {% katex %} y = b + t \sin \theta {% endkatex %} とおいている</q> とありますが、これはおそらく p.39 の <q>{% katex %} l {% endkatex %} には {% katex %} x {% endkatex %} 軸 {% katex %} y {% endkatex %} 軸と同じ間隔で目盛りが振られます。</q> を指しています。
...
発散が導入され、発散が直交座標の取り方に依らない値であることが示されています。
...
回転が導入され、回転が直交座標の取り方に依らない値であることが示されています。

p.52 の {% katex %} \displaystyle{ \theta_y = \frac{V_y(x + \triangle x, y) - V_y(x, y)}{\triangle x} } {% endkatex %} について補足しておきます。:

平面上で原点を中心とする半径 {% katex %} r {% endkatex %} の円周上を質点が初期位相 {% katex %} \theta_0 {% endkatex %} 、角速度 {% katex %} \omega {% endkatex %} で等速円運動しているとき、時間 {% katex %} t {% endkatex %} における位置を {% katex %} P {% endkatex %} とすると、質点の {% katex %} y {% endkatex %} 座標は、
...
p.56-p.57 間で符号が反転している気がする。

節の最後にラプラシアンが導入されています。

## §8 勾配，発散，回転の公式

勾配、発散、回転、ラプラシアンの定義が纏められ、関聯する公式が紹介されています。

## §9 ポテンシャル

スカラーポテンシャルとベクトルポテンシャルが導入されています。
...
曲線の接線ベクトルについて復習した後、ベクトル場の線積分が導入されています。

例題を通して、あるスカラー場の勾配であるベクトル場の線積分は、始点と終点が等しければ、経路に依らず一定の値であることが説明されています。

<!-- TODO: pp.76-77 の補足 -->

さらに、回転が {% katex %} \boldsymbol{0} {% endkatex %} であるベクトル場はあるスカラー場の勾配なので、回転が {% katex %} \boldsymbol{0} {% endkatex %} であるベクトル場の線積分も、始点と終点が等しければ、経路に依らず一定の値である、ということが示されています。

## §12 曲面の面積

空間中の曲面が2つのパラメーターで表せることが簡単に説明され、曲面を一方のパラメーターで偏微分すると、接ベクトルという、他方のパラメーターを固定した曲線の接線ベクトルのようなものが得られることが説明されています。

接ベクトルを用いて曲面上の領域の面積を得る公式が紹介されています。証明は与えられていませんが、例題として球面の面積を計算させることで、経験に反さなさそうなことが確認できるようになっています。

p.81 にはエラッタ[^fn-2]済みの誤植があります。

[^fn-2]: [https://www.beret.co.jp/errata/book/638](https://www.beret.co.jp/errata/book/638)

p.82 以降、いくつかの {% katex %} d \boldsymbol{S} {% endkatex %} が {% katex %} dS {% endkatex %} と書かれています。

<!-- TODO: 問題 1.30 のスカラー場 f を可視化する -->

## §13 ベクトル場の面積分

ベクトル場の面積分とストークスの定理、ガウスの発散定理が導入されています。

p.88 で {% katex %} C_1 {% endkatex %} 、 {% katex %} C_2 {% endkatex %} 、 {% katex %} C_3 {% endkatex %} に使われている {% katex %} x {% endkatex %} 、 {% katex %} y {% endkatex %} はパラメーターです。

p.89 の <q>{% katex %} \displaystyle{ \int_a^0 A_x \left( x, b \left( 1 - \frac{x}{a} \right) , 0 \right) dx + \int_0^b A_y \left( a \left( 1 - \frac{y}{b} \right) , y, 0 \right) dy } {% endkatex %}</q> は、 pp.72-73 で導入された {% katex %} \displaystyle{ \int_C \boldsymbol{F} \cdot d \boldsymbol{r} = \int_{x(a)}^{x(b)} F_x(x, y) dx + \int_{y(a)}^{y(b)} F_y(x, y) dy } {% endkatex %} の適用です。

p.89 の {% katex %} \displaystyle{ \int_0^{a \left( 1 - \frac{y}{b} \right)} } {% endkatex %} と {% katex %} \displaystyle{ \int_0^a } {% endkatex %} は積分の方向が逆で、結論の符号も反転している気がする。法線ベクトルを右ねじの向きに取る理由もよく分からない[^fn-3]ので、後日追記します。

[^fn-3]: これを書いているときは「一貫性があれば十分」と思っています。

さらに、発散が {% katex %} 0 {% endkatex %} であるベクトル場の、閉曲線を境界に持つ曲面での面積分は、閉曲線が等しければ、領域に依らず一定の値であることが示されています。 §11 で導入された定理の面積分版です。

## §14 逆2乗法則についての計算

公式が2つ導入されています。公式 1.35 は §16 で何度か参照されます。

p.98 で {% katex %} \displaystyle{ \int_S dS = 4 \pi r^2 } {% endkatex %} （{% katex %} S {% endkatex %} は半径 {% katex %} r {% endkatex %} の球面）が使はれています。確認しておきます。:
...
1次元の波動方程式の一般解と3次元の波動方程式の解の一つが示されています。

本には <q>1次</q> 、 <q>3次</q> と書かれているけど、これは次数ではない気がする。

## §16 ポアソン方程式

ポアソン方程式 {% katex %} \Delta \varphi(\boldsymbol{x}) = -f(\boldsymbol{x}) {% endkatex %} の特殊解 {% katex %} \displaystyle{ \varphi(\boldsymbol{x}) = \frac{1}{4 \pi} \int_V \frac{f(\bold{y})}{|\boldsymbol{y} - \boldsymbol{x}|} d \boldsymbol{y} } {% endkatex %} が紹介されています。 {% katex %} V {% endkatex %} は {% katex %} \mathbb{R}^3 {% endkatex %} 上の領域、 {% katex %} \boldsymbol{x} {% endkatex %} は {% katex %} V {% endkatex %} 上の定点、 {% katex %} \boldsymbol{y} {% endkatex %} は {% katex %} V {% endkatex %} です。

p.107 に <q>{% katex %} \displaystyle{ \Delta \left( \frac{1}{|\boldsymbol{y} - \boldsymbol{x}|^{1 - \alpha}} \right) } {% endkatex %} の符号が負</q> とありますが、こゝでは {% katex %} \alpha < 1 {% endkatex %} が仮定されています。 {% katex %} \alpha < 1 {% endkatex %} としても {% katex %} 0 < \alpha - 0 < \delta {% endkatex %} な {% katex %} \delta > 0 {% endkatex %} は存在します。
...
p.110 の <q>(1.21) より</q> とある変形では偏微分の順序が交換されていますが、中扉に <q>微分と積分の順序も，2変数関数の偏微分の順序も常に交換可能とします。</q> とあったので、今回は気にせず読み進めます。気が向いたら確認します。

p.111 で {% katex %} \displaystyle{ \frac{\partial^2}{\partial u^2} f(\boldsymbol{y}, u) } {% endkatex %} の項の係数にしれっと {% katex %} \displaystyle{ -\frac{1}{c^2} \frac{1}{|\boldsymbol{y} - \boldsymbol{x}|} } {% endkatex %} がありますが、 {% katex %} \displaystyle{ \frac{\partial^2}{\partial t^2} \left( f \left( \boldsymbol{y}, t + \frac{|\boldsymbol{y} - \boldsymbol{x}|}{c} \right) \frac{1}{|\boldsymbol{y} - \boldsymbol{x}|} \right) } {% endkatex %} は文中では計算されていないので、確認しておきます。:
...
{% katex %} \displaystyle{ \frac{\partial}{\partial t} \frac{1}{|\boldsymbol{y} - \boldsymbol{x}|} } = 0 {% endkatex %} と {% katex %} \displaystyle{ \frac{d^2}{d x^2} f(g(x)) = \frac{d}{dx} \frac{d}{dx} f(g(x)) = \frac{d}{dx} \left( \frac{d}{dg} f(g(x)) \frac{d}{dx} g(x) \right) = \frac{d}{dx} \frac{d}{dg} f(g(x)) \frac{d}{dx} g(x) + \frac{d}{dg} f(g(x)) \frac{d^2}{d x^2} g(x) } {% endkatex %} より
...
p.112 で {% katex %} \displaystyle{ -f \left( \boldsymbol{x}, t + \frac{|\boldsymbol{x} - \boldsymbol{x}|}{c} \right) } {% endkatex %} になっているのは、 {% katex %} \varepsilon {% endkatex %} を {% katex %} +0 {% endkatex %} に飛ばすと {% katex %} \boldsymbol{y} = \boldsymbol{x} {% endkatex %} となるからです。

## §17 変分法

汎関数と極値関数が簡単に紹介され、変分法とオイラー・ラグランジュの方程式が導入されています。

## §18 アインシュタインの縮約記法

アインシュタインの縮約記法と関聯用語、クロネッカーのデルタが導入されています。
