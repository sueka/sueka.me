...
<!-- 以下、この記事では、これらのパラメーターを {% katex %} t_x {% endkatex %} 、 {% katex %} t_y {% endkatex %} と書きます。 -->
...
p.112 で {% katex %} \displaystyle{ -f \left( \boldsymbol{x}, t + \frac{|\boldsymbol{x} - \boldsymbol{x}|}{c} \right) } {% endkatex %} になってゐるのは、 {% katex %} \epsilon {% endkatex %} を {% katex %} +0 {% endkatex %} に飛ばすと {% katex %} \boldsymbol{y} = \boldsymbol{x} {% endkatex %} となるからです。
