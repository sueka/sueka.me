...
シェル命令 ( `xsel` `pbcopy` `clip` ) に差異があること、 Java 等による一般的な方法には入出力による副作用があること、および Hackage の [Clipboard](//hackage.haskell.org/package/Clipboard) は文字列しか扱へないことから、 Java によるクリップボードアクセスを `scalaz.effect.IO` で安直に安全にしてみたものです。
