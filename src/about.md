...
ブログらしいナビゲーション機能はありませんが、代はりに[Atomフィード]({{ '~/feed.xml.njk' | url(true) }})で全てのページを配信してゐます。

## 作者

Suekaと言ひます。東京でウェブアプリケーションを開発してゐます。<time datetime="2018-10">最近</time>の関心事はコンピューター・アーキテクチャと型理論です。

### 個人識別のための情報

実名（`氏 名`形式、UTF-8）のSHA-256ダイジェスト
