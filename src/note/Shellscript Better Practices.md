...
- `sh` や `nice` のやうな、ユーティリティを実行するユーティリティは、実行するユーティリティが[126]{.upright}や[127]{.upright}を返すやうな場合、その値をそのまゝ返す。
- `test` や `grep` のやうな、真偽値を返すユーティリティは、真を[0]{.upright}、偽を[1]{.upright}で表す。
