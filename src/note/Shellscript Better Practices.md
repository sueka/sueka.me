...
*while 文や for 文にパイプしない*。`|` で接続されたコマンドはサブシェルで実行される[^29][^30]ため、パイプラインで実行される変数代入やビルトインコマンドは現在の環境には影響しない[^31]。例へば、ファイルリストにある `.` で始まるファイルの個数を数へるプログラム[^32]を
...
シェルの条件は、真なら[0]{.upright}を、偽なら[0]{.upright}でない値を返すコマンドである。if 文の `if` `elif` の右の部分と while 文の `while` の右の部分は複合コマンドである[^36]。
...
*無駄な cat を避ける*。例へば、`cat |` といふコード片は、`cat` は標準入力を標準出力に移し、パイプ `|` は左辺のコマンドの標準出力を右辺のコマンドの標準入力に繋げるだけなので、いつでも省略できる。
