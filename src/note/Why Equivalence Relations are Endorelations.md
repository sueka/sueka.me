...
アップキャストは大抵安全なので、互ひに部分型関係にある型の項の同値関係は、レシーバーと実引数のどちらかをアップキャストすれば、（型）安全に調べられる。

しかし、型指定は値に影響を及ぼさないので、もし実際に、より狭い型の項をアップキャストすれば、互ひに部分型関係にある型の項の同値関係が調べられるとすれば、部分型の同値関係の検査は上位型の同値関係の検査としても振る舞ふ必要がある。
