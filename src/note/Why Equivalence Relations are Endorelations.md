...
とある。原文は [Object (Java SE 9 & JDK 9 )](https://docs.oracle.com/javase/9/docs/api/java/lang/Object.html){lang=en}。
...
- 反射的であり、
- 対称的であり、
- *[2]{.upright}つの値が等価なら、それらの値は同じ解釈を持ち*、かつ
- [2]{.upright}つの値が同じ解釈を持つなら、それらの値は等価である
...
計算機プログラミングには型指定といふ仕組みがある。型指定は、C++ や Java などの言語ではキャストとも呼ばれる、プログラムの中で、項の型を指定する機能である。型を指定された項はその型の項として振る舞ふ。型 𝑇 と 𝑇 の部分型 𝑆 があるとき、型 𝑆 の項に型 𝑇 を指定することをアップキャスト、型 𝑇 の項に型 𝑆 を指定することをダウンキャストと言ふ。

アップキャストは大抵安全なので、互ひに部分型関係にある型の項の同値関係は、レシーバーと実引数のどちらかをアップキャストすれば、安全に調べられる。

しかし、もし実際に、より狭い型の項をアップキャストすれば、互ひに部分型関係にある型の項の同値関係が調べられるのだとすれば、部分型の同値関係の検査は上位型の同値関係の検査としても振る舞ふ必要がある。
...
よって、同値関係の検査を値の等価性として提供するなら、インスタンス化可能なクラスを拡張して、同値関係にとって<em><ruby>意味のある<rt lang="en">significant</ruby></em>フィールドを追加することはできない。『Effective Java 第[3]{.upright}版』p.46 には次のやうにある:
...
といふわけで、以下では、部分型は上位型にない<ruby>意味のある<rt lang="en">significant</ruby>フィールドを持たないものとする。

---

サブクラス化において、<ruby>意味のある<rt lang="en">significant</ruby>フィールドを追加しないといふことは、部分型の値の集合が上位型の値の集合の部分集合となるといふことである[^7]。

[^7]: 意味のないフィールドは存在しないものとする。Java の `hashCode` を仲介すると考へれば、無理のある仮定ではない。
