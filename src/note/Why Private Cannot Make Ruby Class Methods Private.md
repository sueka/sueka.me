...
としてもうまく動作する。この設計も `private` の場合と同様に危険だが、クラスメソッド定義の中ではクラスが暗黙のレシーバーになるので、仮にこのメソッドの可視性が [private]{lang=en} でも、次のやうにすれば、クラス定義の外でクラスメソッドを [private]{lang=en} にすることができてしまふ:
